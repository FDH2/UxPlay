...
6.  Assuming no error in either of these, you will have built the uxplay
    ...
    `cmake --install . --prefix $HOME/../../ucrt64`,  (replace "ucrt64" by "mingw64" if you chose not to use UCRT)

    You can later uninstall uxplay by returning to the build directory and running

    `ninja uninstall`

    (This assumes that certain files in the build directory have not been deleted). 
...
changes: d3d11 will used by default until this is fixed).
...
(replace `ucrt64` by ``mingw64`` if you built UxPlay in the MINGW64 environment).
