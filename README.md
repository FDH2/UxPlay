...
-   Support for **service discovery using a Bluetooth LE "beacon"** for both Linux/\*BSD and Windows (as an alternative to Bonjour/Rendezvous DNS-SD
    ...
    later "dongle" can be used). The beacon is managed by a Python3 script `uxplay-beacon.py`: four implementations of Bluetooth LE advertising are available
    ...
    host system) that runs on all systems including macOS and *BSD), and a low-level **HCI** module (Linux and FreeBSD only) that
    accesses the Host Controller Interface (but users need enhanced privileges to use this). The beacon
...
* **BleuIO** for the [BleuIO (or BleuIO Pro) USB dongle](https://www.bleuio.com)  with its own on-board Bluetooth-LE Stack that
    ...
    group `dialout` (``dialer`` on FreeBSD).

* **HCI**  for Linux (uses utiities `hcitool` and ``hciconfig``)  or FreeBSD (uses
    ...
    this can be configured by the system administrator using `visudo`, but may have security implications.   Use visudo to create a file `hciusers`
    in `/etc/sudoers.d/` (Linux)
    or `/usr/local/etc/sudoers.d` (FreeBSD), containing the
    line `%hciusers ALL=(ALL) NOPASSWD: <hcitools>` (where  ``<hcitools>``
    is "`/usr/bin/hcitool, /usr/bin/hciconfig`" (Linux) or "``/usr/sbin/hccontrol``" (FreeBSD).
...
* `--device x` which allows overiding automatically-made choices of serial ports (**BleuIO**) or hci device nodes (**HCI**). 
...
possibly could be used to adapt the python3  **HCI** module to support macOS as well (**implementations welcome!**).
