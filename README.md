...
3.  [MSYS2 packages](https://packages.msys2.org/package/) are installed
    ...
    * _NEW: MSYS2 now recommends using the newer UCRT64 terminal environment (which uses the newer Microsoft
    ...
    the older version of this README gave
    instructions for the MINGW64 environment: if you still wish to use the legacy MSVCRT library, to support older Windows versions,
    use  a "MSYS2 MINGW64" terminal, modify mingw-w64-ucrt-x86_64-* package names to mingw-w64-x86_64-*, (just omit "-ucrt") and install UxPlay
    ...
    `C:\msys64\ucrt64`, (or ``C:\msys64\mingw64`` if you opted to not use UCRT). We
...
6.  Assuming no error in either of these, you will have built the uxplay
    ...
    `cmake --install . --prefix ucrt64`,  (replace "ucrt64" by "mingw64" if you chose not to use UCRT)
...
`glimagesink`, `gtksink`.    If you do not specify the videosink, the d3d11videosink will be used (users have
reported segfaults of the newer d3d12  videodecoder on certain older Nvidia cards when the image resolution
changes: d3d11 will set as default until this is fixed  in GStreamer).
...
-   With Direct3D 11.0 or greater, various options can be set
    ...
    toggle in and out of fullscreen mode is added.
    with the Alt-Enter key combination is added.
...
(or `C:\msys64\mingw64\bin\uxplay`if you built UxPlay in the MINGW64 environment).
