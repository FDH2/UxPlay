...
static audio_renderer_t *audio_renderer_type[NFORMATS];
...
void audio_renderer_init(logger_t *render_logger, const char* audiosink, const bool* audio_sync, const bool* video_sync, const char *artp_pipeline) {
    ...
    for (int i = 0; i < NFORMATS ; i++) {
        audio_renderer_type[i] = (audio_renderer_t *)  calloc(1,sizeof(audio_renderer_t));
        g_assert(audio_renderer_type[i]);
        ...
        audio_renderer_type[i]->pipeline  = gst_parse_launch(launch->str, &error);
	if (error) {
          g_error ("gst_parse_launch error (audio %d):\n %s\n", i+1, error->message);
          g_clear_error (&error);
        }

        g_assert (audio_renderer_type[i]->pipeline);
        gst_pipeline_use_clock(GST_PIPELINE_CAST(audio_renderer_type[i]->pipeline), clock);
        audio_renderer_type[i]->bus = gst_element_get_bus(audio_renderer_type[i]->pipeline);
        audio_renderer_type[i]->appsrc = gst_bin_get_by_name (GST_BIN (audio_renderer_type[i]->pipeline), "audio_source");
        audio_renderer_type[i]->volume = gst_bin_get_by_name (GST_BIN (audio_renderer_type[i]->pipeline), "volume");
        switch (i) {
        case 0:
            caps =  gst_caps_from_string(aac_eld_caps);
            audio_renderer_type[i]->ct = 8;
            format[i] = "AAC-ELD 44100/2";
            break;
        case 1:
            caps =  gst_caps_from_string(alac_caps);
            audio_renderer_type[i]->ct = 2;
            format[i] = "ALAC 44100/16/2";
            break;
        case 2:
            caps =  gst_caps_from_string(aac_lc_caps);
            audio_renderer_type[i]->ct = 4;
            format[i] = "AAC-LC 44100/2";
            break;
        case 3:
            caps =  gst_caps_from_string(lpcm_caps);
            audio_renderer_type[i]->ct = 1;
        ...
        g_object_set(audio_renderer_type[i]->appsrc, "caps", caps, "stream-type", 0, "is-live", TRUE, "format", GST_FORMAT_TIME, NULL);
        gst_caps_unref(caps);
...
void audio_renderer_stop() {
    if (audio_renderer) {
        gst_app_src_end_of_stream(GST_APP_SRC(audio_renderer->appsrc));
        gst_element_set_state (audio_renderer->pipeline, GST_STATE_NULL);
        audio_renderer = NULL;
    }
}

static void get_audio_renderer_type(unsigned char *ct, int *id) {
    render_audio = FALSE;
    *id = -1;
    for (int i = 0; i < NFORMATS; i++) {
        if (audio_renderer_type[i]->ct == *ct) {
	    *id = i;
            break;
        }
    }
...
void  audio_renderer_start(unsigned char *ct) {
    int id = -1;
    get_audio_renderer_type(ct, &id);
    if (id >= 0 && audio_renderer) {
        if(*ct != audio_renderer->ct) {
            gst_app_src_end_of_stream(GST_APP_SRC(audio_renderer->appsrc));
            gst_element_set_state (audio_renderer->pipeline, GST_STATE_NULL);
            logger_log(logger, LOGGER_INFO, "changed audio connection, format %s", format[id]);
            audio_renderer = audio_renderer_type[id];
            gst_element_set_state (audio_renderer->pipeline, GST_STATE_PLAYING);
            gst_audio_pipeline_base_time = gst_element_get_base_time(audio_renderer->appsrc);
        }
    } else if (id >= 0) {
...
void audio_renderer_render_buffer(unsigned char* data, int *data_len, unsigned short *seqnum, uint64_t *ntp_time) {
    ...
    if (data_len == 0 || audio_renderer == NULL) return;
    ...
    if (valid) {
        gst_app_src_push_buffer(GST_APP_SRC(audio_renderer->appsrc), buffer);
    } else {
        logger_log(logger, LOGGER_ERR, "*** ERROR invalid  audio frame (compression_type %d) skipped ", audio_renderer->ct);
...
void audio_renderer_set_volume(double volume) {
    ...
    g_object_set(audio_renderer->volume, "volume", volume, NULL);
}
...
void audio_renderer_destroy() {
    audio_renderer_stop();
    for (int i = 0; i < NFORMATS ; i++ ) {
        gst_object_unref (audio_renderer_type[i]->bus);
        audio_renderer_type[i]->bus = NULL;
        gst_object_unref (audio_renderer_type[i]->volume);
        audio_renderer_type[i]->volume = NULL;
        gst_object_unref (audio_renderer_type[i]->appsrc);
        audio_renderer_type[i]->appsrc = NULL;
        gst_object_unref (audio_renderer_type[i]->pipeline);
        audio_renderer_type[i]->pipeline = NULL;
        free(audio_renderer_type[i]);
    }
}
...
static gboolean gstreamer_audio_pipeline_bus_callback(GstBus *bus, GstMessage *message, void *loop) {
    ...
    case GST_MESSAGE_ERROR: {
        ...
        g_error_free(err);
        g_free(debug);
        if (audio_renderer->appsrc) {
            gst_app_src_end_of_stream (GST_APP_SRC(audio_renderer->appsrc));
        }
        gst_bus_set_flushing(bus, TRUE);
        gst_element_set_state (audio_renderer->pipeline, GST_STATE_READY);
...
unsigned int audio_renderer_listen(void *loop, int id) {
    ...
    return (unsigned int) gst_bus_add_watch(audio_renderer_type[id]->bus,(GstBusFunc)
