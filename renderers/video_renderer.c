...
/* Platform-agnostic hook for embedding video into a window the *app*
 * created, via GstVideoOverlay's standard "prepare-window-handle" bus
 * message. Nothing currently calls video_renderer_set_window_handle() on
 * X11/Win32/macOS/Wayland - they let their sinks create their own top-level
 * window as before - so this stays a no-op there. Android has no concept of
 * an app creating an arbitrary top-level window: glimagesink must be handed
 * a real ANativeWindow, supplied by JNI code (see uxplay_jni.cpp) once the
 * app's SurfaceView is ready. The bus watch has to be synchronous: by the
 * time an async watch would see this message, the sink has already moved on
 * without a window.
 */
