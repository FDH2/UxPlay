...
#define NCODECS  4   /* renderers for h264, h265, hls, and jpeg images */
...
void video_renderer_init(logger_t *render_logger, const char *server_name, videoflip_t videoflip[2], const char *parser, const char * rtp_pipeline,
    ...
    for (int i = 0; i < n_renderers; i++) {
        ...
        if (i == type_hls) {
            ...
            logger_log(logger, LOGGER_DEBUG, "GStreamer video pipeline %d: using Playbin version %u for HLS streamed video",
                       i + 1, playbin_version);
        } else if (i == type_264) {
...
void video_renderer_start(const char *uri) {
    ...
    /* when not hls, start h264, h265 and jpeg pipelines; will shut down the "wrong" one when we know the codec */
    hls_video = false;
    for (int i = 0; i < n_renderers; i++) {
        if (i == type_hls) {
            continue;
...
int video_renderer_choose_codec (bool video_is_jpeg, bool video_is_h265, bool video_is_hls) {
    ...
    if (!video_is_hls) { //hls renderer is already set 
