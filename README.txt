...
6.  Assuming no error in either of these, you will have built the uxplay
    ...
    `cmake --install . --prefix $HOME/../../ucrt64`, (replace "ucrt64"
    by "mingw64" if you chose not to use UCRT)
    ...
    (This assumes that certain files in the build directory have not
    been deleted).
...
when the image resolution changes: d3d11 will used by default until this
is fixed).
...
in the Windows Terminal, with `C:\msys64\ucrt64\bin\uxplay`. (replace
`ucrt64` by `mingw64` if you built UxPlay in the MINGW64 environment).
