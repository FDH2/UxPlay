...
-   Support for **service discovery using a Bluetooth LE "beacon"** for
    ...
    mDNS/DNS-SD service discovery) was introduced in v1.73
    and improved in 1.73.4 - 1.73.6. This can be used on networks that
    do not allow mDNS multicast.\*\* The user must run
...
queries on the server. If UxPlay is seen, but the client fails to
...
Service Discovery is an alternative to mDNS/DNS-SD, and can be used on
networks that don't allow mDNS multicast. See [instructions
below](#bluetooth-le-beacon-setup).
